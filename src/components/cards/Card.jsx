...
const Card = (data) => {
  ...
  return (
    <div className="sm:w-[80%] w-full relative text-center sm:h-[90%] h-full flex flex-col rounded-3xl  sm:text-[2.5rem] text-[1.5rem]  bg-[rgba(26,36,58,0.96)] ">
      <div className="flex flex-row absolute items-center">
        ...
        <div>
          ...
          <img
            src={sound}
            alt="404"
            className="sm:w-20 sm:h-16 ml-10 w-12 cursor-pointer transition-all hover:scale-105"
      ...
      <hr />
      <div className="flex flex-col pb-20 overflow-scroll no-scrollbar">
        <div>{romaji}</div>
        <div className={`${!sentence && "hidden"} text-[#FF851B]`}>
          {japanese_elements[5]}
        </div>
        <div className={`${!sentence && "hidden"}`}>{sentence_romaji}</div>
        <div className={`${!sentence && "hidden"} text-[#FF851B]`}>
          {japanese_elements[6]}
        </div>
      </div>
