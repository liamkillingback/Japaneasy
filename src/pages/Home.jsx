...
import { Card, Selection, Menu } from "../components";
  ...
  return (
    <div className="w-full h-full items-center justify-center flex absolute top-0 left-0">
      <div
        className={`${
          intro && "hidden"
        } md:w-[60%] w-[90%] sm:h-[85%] h-[78%] sm:mt-24 rounded-xl mb-10 mt-12 flex flex-col items-center justify-center fixed bg-[rgba(13,18,29,0.96)]`}
      >
      ...
      >
        ...
        <button className="text-[2rem] text-black bg-[#5ce632] p-3 rounded-full m-10 hover:scale-105 transition-all" onClick={() => setIntro(!intro)}>Get started!</button>
      </div>
    </div>
  );
};
